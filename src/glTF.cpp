...
void write_glTF__triangle_mesh(std::string filename, GEO::Mesh& M) {
    ...
    size_t buffer_indices_length = 4*3*M.facets.nb(); // number of bytes
    ...
    FOR(f,M.facets.nb()) { // for each facet (triangle) index
        memcpy(buffer_0.data.data() + f*12 + 0, facet_vertex_index_ptr(M,f,0),4);
        memcpy(buffer_0.data.data() + f*12 + 4, facet_vertex_index_ptr(M,f,1),4);
        memcpy(buffer_0.data.data() + f*12 + 8, facet_vertex_index_ptr(M,f,2),4);
    }
    // write vertices (= 3D coordinates)
    FOR(v,M.vertices.nb()) { // for each vertex index
        memcpy(buffer_0.data.data()+buffer_coordinates_start+v*12+0,static_cast<void*>(M.vertices.single_precision_point_ptr(v)),12); // write x,y,z from float* getter
    }
    ...
    accessor_0_triangles_vertices.count = M.facets.nb()*3; // how many indices
    accessor_0_triangles_vertices.type = TINYGLTF_TYPE_SCALAR;
    // range of indices : [ 0 : M.facets.nb()*3-1 ]
    accessor_0_triangles_vertices.maxValues.push_back(M.vertices.nb()-1); // max value of all indices in the buffer view
    ...
    accessor_1_vertices_coordinates.count = M.vertices.nb(); // how many points
